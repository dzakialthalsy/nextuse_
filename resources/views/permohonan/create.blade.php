    ...
    <div class="bg-gray-50 min-h-screen py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                ...
                <div
                    class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-xl border border-teal-100 bg-teal-50/60 p-4">
                    <div>
                ...
                @if($latestRequest)
                    <div class="p-4 rounded-xl border border-amber-200 bg-amber-50 text-sm text-amber-900">
                        ...
                        <p class="mb-1">Pengajuan terakhir: <span
                                class="font-medium">{{ $latestRequest->created_at->format('d M Y H:i') }}</span></p>
                        <p>Status: <span class="capitalize font-medium">{{ $latestRequest->status }}</span></p>
                    </div>
                @endif

                <form action="{{ route('item-requests.store', $item->id) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-5">
                    @csrf
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="requested_quantity" id="requested_quantity" min="1"
                            max="{{ $item->jumlah }}" value="{{ old('requested_quantity', 1) }}"
                            class="w-full rounded-xl border-gray-200 focus:border-teal-500 focus:ring-teal-500" required>
                    ...
                    <div class="space-y-3">
                        ...
                        <div id="document-upload-area"
                            class="border-2 border-dashed border-[#009689] bg-teal-50 rounded-[10px] p-6 transition-colors hover:border-teal-600">
                            <input id="surat_kuasa" type="file" name="surat_kuasa" accept=".pdf,.doc,.docx" class="hidden"
                                required>
                            <label for="surat_kuasa" class="flex flex-col items-center gap-3 cursor-pointer">
                                <div class="p-3 bg-white rounded-full shadow-sm border border-[#009689]">
                                    <svg class="h-6 w-6 text-[#009689]" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                        ...
                        <p class="text-xs text-gray-500">Pastikan dokumen telah ditandatangani pimpinan atau penanggung
                            jawab.</p>
                        @error('surat_kuasa')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
