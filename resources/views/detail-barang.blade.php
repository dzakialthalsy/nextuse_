...
@section('content')
    <div class="bg-gray-50 min-h-screen py-10">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-8">
                <a href="{{ url()->previous() }}"
                    class="inline-flex items-center text-sm text-gray-500 hover:text-gray-900 transition">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="aspect-square rounded-2xl overflow-hidden bg-gray-100 mb-4">
                        @if(!empty($photos))
                            <img src="{{ asset('storage/' . $photos[0]) }}" alt="{{ $item->judul }}"
                                class="w-full h-full object-cover">
                        @else
                            <img src="{{ asset('images/product-placeholder.jpg') }}" alt="{{ $item->judul }}"
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            @foreach($photos as $photo)
                                <div class="h-20 rounded-xl overflow-hidden border border-gray-100">
                                    <img src="{{ asset('storage/' . $photo) }}" alt="{{ $item->judul }}"
                                        class="w-full h-full object-cover">
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="flex flex-wrap gap-3 text-sm text-gray-600 mb-4">
                            ...
                            <span
                                class="px-3 py-1 rounded-lg border border-gray-200 font-semibold capitalize">{{ $item->kondisi }}</span>
                        ...
                        <div class="mt-6 border-t border-gray-100 pt-6">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500">
                                        {{ $item->organization->organization_type ?? 'Organisasi' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($hasGiveaway)
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <span class="text-xs px-3 py-1 rounded-full bg-gray-100 text-gray-600 font-medium">Berbagi
                                        Barang</span>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                        ...
                        @if($isLoggedIn && $isReceiver && !$isOwner)
                            <p class="text-sm text-gray-600">Lengkapi Surat Permohonan Hibah sesuai template resmi, lalu ajukan
                                permohonan Anda kepada pemilik barang.</p>
                        ...
                        @elseif(!$isLoggedIn)
                            ...
                            <a href="{{ route('login') }}"
                                class="inline-flex items-center justify-center px-6 py-3 border border-teal-200 text-teal-700 font-semibold rounded-xl hover:bg-teal-50 transition shadow-sm">
                                Masuk untuk Mengajukan
                            </a>
                        @elseif($isOwner)
                            <p class="text-sm text-gray-600">Ini adalah barang yang Anda bagikan. Permohonan tidak diperlukan.
                            </p>
                        @else
                            <p class="text-sm text-gray-600">Fitur permohonan hanya tersedia bagi organisasi penerima
                                terverifikasi.</p>
                        @endif
                    </div>
